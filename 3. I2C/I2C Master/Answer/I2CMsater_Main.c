	...
	{	
														...
														// 可以用軟體模擬來檢查是否為準確的 1 ms
		{
			OneSecond = 0 ;		
				...
				do
				{
					if ( !I2C_ACKPolling(0xA0 )) 		// Return value : 0= ACK  , 1 = NACK
						{		
							EEPROM_Data1 = EEPROM_ByteRead( 0xA0  , 0x10  ) ;
							EEPROM_Data2 = EEPROM_ByteRead( 0xA0 , 0x20 ) ;	
							WaitLoop = 1000 ;
						}
					else
							WaitLoop ++ ;
			...
			put_Num_LCD( ADC_Value >> 2 ) ;				// 將類比轉換結果以十進位數字顯示至液晶顯示器
				...
				do
				{
					if ( !I2C_ACKPolling(0xA0 )) 		// Return value : 0= ACK  , 1 = NACK
						{		
							EEPROM_ByteWrite( 0xA0 , 0x10 , ADC_Value >> 2 ) ;
							WaitLoop = 1000 ;
						}
					else
							WaitLoop ++ ;
			...
			put_Num_LCD( ADC_Value >> 2 ) ;				// 將類比轉換結果以十進位數字顯示至液晶顯示器	
				...
				do
				{
					if ( !I2C_ACKPolling(0xA0 )) 		// Return value : 0= ACK  , 1 = NACK
						{		
							EEPROM_ByteWrite( 0xA0 , 0x20 , ADC_Value >> 2 ) ;
							WaitLoop = 1000 ;
						}
					else
							WaitLoop ++ ;

...
int		EEPROM_ByteWrite(unsigned char CMD , unsigned int Addr , unsigned char Data ) 
{
		IdleI2C( ) ;
		StartI2C( ) ;
		...
		IFS0bits.MI2CIF = 0 ;
		MasterWriteI2C((unsigned char)((Addr>>8) & 0x00ff)) ;
		...
		IFS0bits.MI2CIF = 0 ;
		MasterWriteI2C((unsigned char)(Addr & 0x00ff)) ;
		while(! IFS0bits.MI2CIF ) ;
...
int		EEPROM_ByteRead(unsigned char CMD , unsigned int Addr  )
{
